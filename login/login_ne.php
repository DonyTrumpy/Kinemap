...
<html lang="en">
  ...
  .popup.active .content {
	...
	.popup.active .content{width:80%;margin-top: 40px;height:510px;
	}}
...
@media screen and (max-width: 450px) {
	#ForgotPassword{font-size: 10px;margin-left: -25px;}}
...
#role{border-radius: 12px;background:#2E2E2E;color:#fff;position: absolute;top:52%; left:57%;}
    ...
    <div class="container">
		...
		<form class="form-signin" method="post" action="../source/loginproc.php">
			<h3 id="Title"> Login</h3><br><br><br><br>
			<label for="username" class="sr-only"></label>
			<input type="text" id="username" name ="username" class="form-control mb-2" placeholder="Gebruikersnaam" required autofocus><br>
			<label for="password" class="sr-only"></label>
			<input type="password" id="password" name ="pass" class="form-control" placeholder="Wachtwoord" required> <br><br><br><br><br>
			<label id="Rolee">Selecteer uw rol</label>
			<select name ="role" id="role">
				<option value="A">Admin</option>
					<option value="U" selected>Gebruiker</option>
			</select>

			<button class="btn btn-md btn-primary btn-block" type="submit" name="submit" id="login">Inloggen</button>
			<br><br>
			  <a href="../source/contact_us_ne.php" id="ForgotPassword"> Wachtwoord vergeten? </a>
			<a href="../source/select_plans_ne.php" id="SignUp">Inschrijven</a>
		</form>
  ...
  if(isset($_GET['s'])){
      $s = $_GET['s'];
		switch ($s){
			...
			case 'lf':
				echo  '<script type="text/javascript" language="javascript">
				swal("Error", "Het is mogelijk dat uw rol anders is of dat uw e-mail / wachtwoord onjuist is. Probeer het a.u.b. opnieuw.", "error");</script>';
			break;
			case 'fl':
				echo  '<script type="text/javascript" language="javascript">
				swal("Error", "Sorry, je moet eerst inloggen", "error");</script>';
			break;
			case 'success':
				echo  '<script type="text/javascript" language="javascript">
				swal("Success", "Succesvol geregistreerd. Log in met inloggegevens", "success");</script>';
			break;
			default:
		}
    }
