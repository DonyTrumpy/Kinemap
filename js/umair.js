...
$(document).ready(function () {
	...
	$(
		"#FHomeLB, #FOnlineLB, #FConventionLB, #one, #two, #three, #four, #five, #six, #seven, #eight, #nine, #ten, #eleven, #twelve, #thirteen, #fourteen, #fifteen, #sixteen, #seventeen, #eighteen, #nineteen, #twenty, #twentyone, #twentytwo, #twentythree, #twentyfour, #twentyfive"
	).click(function (e) {
		if ($(window).width() < 880) {
	...
	$("#GoCity, #GoAddress, #GoName").click(function (e) {
		if ($(window).width() < 880) {
            ...
            // if ($("#questionMarkId").is(":visible")) {
				...
				if($(this).hasClass('visibleMark')){
					$(this).removeClass('visibleMark');
					$("#questionMarkId").hide();
				}else{
					$(".visibleMark").removeClass('visibleMark');
					$(this).addClass('visibleMark');
					$("#questionMarkId").show();
				}
...
function getAllData() {
	$.getJSON("../abc/fetchData.php", function (data) {
		...
		vectorLayer
			.getSource()
			.getSource()
			.addFeatures(geoJSONFormat.readFeatures(data));
	});
}
...
function onMapClick(evt) {
	...
	if (!feature) {
		...
		if (window.screen.availWidth < 450) {
			...
			content.innerHTML =
				'go see <a href="#TPP" title="Back to Middle">' +
				...
				switch (id) {
					...
					case "6":
						if (
							features[x]
								.get("specialisations")
								.indexOf("Oro-facial | Maxillo-facial") !== -1
						)
							features[x].set("dikhao", set);
						break;
					case "7":
						if (
							features[x]
								.get("specialisations")
								.indexOf("Thérapie respiratoire") !== -1
						)
							features[x].set("dikhao", set);
						break;
					case "8":
						if (
							features[x]
								.get("specialisations")
								.indexOf("Thérapie cardiovasculaire") !== -1
						)
							features[x].set("dikhao", set);
						break;
					case "9":
					...
					case "12":
						if (
							features[x]
								.get("specialisations")
								.indexOf("Thérapie vestibulaire") !== -1
						)
							features[x].set("dikhao", set);
						break;
					case "13":
					...
					case "15":
						if (
							features[x]
								.get("specialisations")
								.indexOf("Dry needling | Acupuncture") !== -1
						)
							features[x].set("dikhao", set);
						break;
					case "16":
						if (
							features[x]
								.get("specialisations")
								.indexOf("Rééducation pelvienne") !== -1
						)
							features[x].set("dikhao", set);
						break;
					case "17":
						if (
							features[x]
								.get("specialisations")
								.indexOf("Drainage lymphatique") !== -1
						)
							features[x].set("dikhao", set);
						break;
					case "18":
					...
					case "20":
						if (
							features[x]
								.get("specialisations")
								.indexOf("Ultrasound | Shockwave") !== -1
						)
							features[x].set("dikhao", set);
						break;
					case "23":
...
function getFilteredFeatures() {
	...
	var list = [],
		query = "";
	if ($("#FHome").is(":checked")) query += "home_therapy='1' and ";
	if ($("#FOnline").is(":checked")) query += "online_therapy='1' and ";
